...
//      variables: { input: { id } },
  ...
  async function sortNotes(event, downOrUp) {
    setDisableButtons(true);
    event.preventDefault();

    let notesSorted = [];
    if(downOrUp) {
      notes.sort((c1, c2) => (c1.temp < c2.temp) ? 1 : (c1.temp > c2.temp) ? -1 : 0);
    }
    else {
      notes.sort((c1, c2) => (c1.temp < c2.temp) ? -1 : (c1.temp > c2.temp) ? 1 : 0);
    }
        ...
        <thead className="table-dark">
          <tr>
            ...
            <th scope="col">
              temp
              {/*<br>*/}
                <Button disabled={disableButtons} size="small" onClick={(event) => sortNotes(event, 1)}>
                  <TiArrowSortedUp/>
                </Button>
                <Button disabled={disableButtons} size="small" onClick={(event) => sortNotes(event, 0)}>
