...
//      variables: { input: { id } },
  ...
  const downloadCSV = (event, csv, filename) => {
    setDisableButtons(true);
    event.preventDefault();
  ...
  function writeCSV(event, filename) {
    if(csvStr == "") {
...
*/
  ...
  return (
    <View className="App">
      ...
      <View as="form" margin="0 0" onSubmit={(event) => writeCSV(event, convertToCSV(), 'test_output.csv')}>
